...
def extract_merchant_from_text(text: str) -> str:
    ...
    for line in lines[:8]:
        ...
        for merchant in common_merchants:
            if merchant in line_clean:
                # Return the line as it is (for proper casing) or the capitalized merchant name
                return line.strip()
    ...
    for line in lines[:8]:
        ...
        address_patterns = [
            ...
            r'^[A-Z][a-z]+$', # Single word city (heuristic)
        ]
        ...
        if re.search(r'\d{1,2}[-/]\d{1,2}[-/]\d{2,4}', line) or re.search(r'\d{1,2}:\d{2}', line):
            continue
    ...
    # Fallback: search for common merchants in entire text
    for merchant in common_merchants:
