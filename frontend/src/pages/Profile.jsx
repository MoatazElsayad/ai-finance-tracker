...
function Profile() {
  ...
  return (
    <div className={`min-h-screen px-6 py-8 transition-colors duration-500 ${isDark ? 'bg-[#0a0e27]' : 'bg-slate-50'}`}>
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-6 animate-in fade-in slide-in-from-top-10 duration-700">
        <div>
          <h1 className="text-3xl md:text-4xl font-black flex items-center gap-4">
            <div className="p-2.5 bg-gradient-to-br from-amber-400 to-amber-600 rounded-2xl shadow-xl shadow-amber-500/20">
              <User className="w-6 h-6 text-white" strokeWidth={2.5} />
            </div>
            <span className={`uppercase tracking-[0.2em] ${isDark ? 'text-white' : 'text-slate-900'}`}>Profile</span>
          </h1>
          <p className={`${isDark ? 'text-slate-400' : 'text-slate-600'} mt-2 text-base font-medium max-w-2xl`}>
      ...
      <main className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-10 duration-700 delay-200">
        {/* Main Profile Card */}
        <div className={`card-unified ${isDark ? 'card-unified-dark' : 'card-unified-light'} !p-6 md:!p-8`}>
          ...
          <div className="relative z-10">
            {/* Avatar & Hero Section */}
            <div className="flex flex-col md:flex-row items-center gap-8 mb-12 pb-12 border-b-2 border-slate-500/10">
              <div className="relative group">
                ...
                <div className={`relative w-32 h-32 rounded-full ${isDark ? 'bg-slate-800' : 'bg-slate-100'} flex items-center justify-center border-4 border-amber-500 shadow-2xl overflow-hidden transform group-hover:scale-105 transition-transform duration-500`}>
                  {user && <UserAvatar user={user} size="w-32 h-32" />}
                </div>
              </div>
              
              <div className="text-center md:text-left">
                <h2 className={`text-3xl font-black tracking-tight ${isDark ? 'text-white' : 'text-slate-900'} mb-1`}>
                  {user ? `${user.first_name} ${user.last_name}` : 'Your Name'}
                </h2>
                <p className={`text-lg font-black ${isDark ? 'text-amber-500' : 'text-amber-600'} mb-4 uppercase tracking-[0.2em]`}>
                  @{user?.username || 'username'}
                </p>
                <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] ${
            ...
            {/* Message banner */}
            {message.text && (
              <div className={`mb-8 p-6 rounded-[2rem] text-center font-black uppercase tracking-[0.2em] animate-in slide-in-from-top-4 duration-500 shadow-xl ${
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-6 mb-12">
              <h3 className={`text-xl font-black tracking-[0.2em] uppercase ${isDark ? 'text-white' : 'text-slate-900'} flex items-center gap-4`}>
                <div className="p-3 bg-amber-500/10 rounded-2xl border-2 border-amber-500/20 shadow-lg shadow-amber-500/5">
                  <User className="w-6 h-6 text-amber-500" strokeWidth={3} />
                </div>
                Account Details
              </h3>
              ...
              {!isEditing ? (
                <button
                  ...
                  className={`flex items-center gap-3 px-6 py-3 rounded-[1.25rem] font-black text-xs uppercase tracking-[0.2em] transition-all duration-500 group active:scale-95 ${
                    isDark 
                ...
                >
                  <Edit className="w-5 h-5 group-hover:rotate-12 transition-transform" strokeWidth={2.5} />
                  Edit Profile
                </button>
              ) : (
                <div className="flex gap-4">
                  <button
                    ...
                    className={`px-6 py-3 rounded-[1.25rem] font-black text-xs uppercase tracking-[0.2em] transition-all duration-500 active:scale-95 ${
                      isDark 
                  ...
                  >
                    <X className="w-5 h-5 inline mr-2" strokeWidth={2.5} />
                    Cancel
                  </button>
                  <button
                    onClick={handleSubmit}
                    disabled={saving}
                    className="btn-primary-unified !rounded-[1.25rem] !py-3 !px-6 shadow-2xl shadow-amber-500/20 disabled:opacity-50 group active:scale-95"
                  >
                    {saving ? (
                      <div className="animate-spin rounded-full h-5 w-5 border-3 border-white border-t-transparent"></div>
                    ) : (
                      <>
                        <Save className="w-5 h-5 group-hover:scale-110 transition-transform" strokeWidth={2.5} />
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <label className={`block text-[10px] font-black uppercase tracking-[0.2em] ml-2 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>First Name</label>
                  {isEditing ? (
                    <input
                      type="text"
                      name="first_name"
                      ...
                      className={`input-unified w-full !text-base ${isDark ? 'input-unified-dark' : 'input-unified-light'}`}
                      placeholder="John"
                    />
                  ) : (
                    <div className={`px-6 py-4 rounded-[1.25rem] font-black text-base transition-all duration-500 ${isDark ? 'bg-slate-800/30 border-slate-700/50 text-white hover:bg-slate-800/50' : 'bg-slate-50/50 border-slate-200 text-slate-900 hover:bg-slate-100/50'} border-2 shadow-inner group`}>
                ...
                <div className="space-y-3">
                  <label className={`block text-[10px] font-black uppercase tracking-[0.2em] ml-2 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Last Name</label>
                  {isEditing ? (
                    <input
                      type="text"
                      name="last_name"
                ...
                <div className="space-y-3">
                  <label className={`block text-[10px] font-black uppercase tracking-[0.2em] ml-2 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Username</label>
                  {isEditing ? (
                    <input
                      type="text"
                      name="username"
                ...
                <div className="space-y-3">
                  <label className={`block text-[10px] font-black uppercase tracking-[0.2em] ml-2 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Email Address</label>
                  <div className={`px-6 py-4 rounded-[1.25rem] font-black text-base transition-all duration-500 ${isDark ? 'bg-slate-800/20 border-slate-700/30 text-slate-500 hover:bg-slate-800/30' : 'bg-slate-50/20 border-slate-200 text-slate-400 hover:bg-slate-100/20'} border-2 flex items-center gap-3 shadow-inner group`}>
                    <Mail className="w-5 h-5 opacity-50 group-hover:text-amber-500 group-hover:opacity-100 transition-all" />
                    <span className="group-hover:text-amber-500 transition-colors">{user?.email || '—'}</span>
                  </div>
                </div>

                <div className="space-y-3">
                  <label className={`block text-[10px] font-black uppercase tracking-[0.2em] ml-2 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Phone Number</label>
                  {isEditing ? (
                    <input
                      type="tel"
                      name="phone"
                ...
                <div className="space-y-3">
                  <label className={`block text-[10px] font-black uppercase tracking-[0.2em] ml-2 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Gender</label>
                  {isEditing ? (
                    <div className="relative group">
                      <select
                        name="gender"
                        ...
                        onChange={handleChange}
                        className={`input-unified w-full !text-base appearance-none pr-10 ${isDark ? 'input-unified-dark' : 'input-unified-light'}`}
                      >
                        <option value="">Select gender</option>
                        <option value="male">Male</option>
                        <option value="female">Female</option>
                      </select>
                      <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-amber-500 transition-transform group-hover:scale-110">
              ...
              {/* Join Date */}
              <div className="pt-8 border-t-2 border-slate-500/10 flex items-center justify-between">
                <div className="flex items-center gap-4 group">
                  <div className={`p-3 rounded-2xl transition-all duration-500 ${isDark ? 'bg-slate-800 text-slate-400 border-slate-700/50 group-hover:bg-slate-700 group-hover:text-amber-500' : 'bg-slate-50 text-slate-500 border-slate-200 group-hover:bg-white group-hover:text-amber-500'} border-2 shadow-sm`}>
                    <Calendar className="w-6 h-6 transition-transform group-hover:scale-110" />
                  </div>
                  <div>
                    <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${isDark ? 'text-slate-500' : 'text-slate-400'} mb-1`}>Member Since</p>
                    <p className={`text-lg font-black transition-colors ${isDark ? 'text-white group-hover:text-amber-500' : 'text-slate-900 group-hover:text-amber-500'}`}>
                      {user?.createdAt 
